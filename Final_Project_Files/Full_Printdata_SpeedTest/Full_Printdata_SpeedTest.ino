...
static const int BNO_RX = 16; // ESP32 RX2  (connected to BNO085 TX)
static const int BNO_TX = 17; // ESP32 TX2  (connected to BNO085 RX)
...
float theta_angle = 0, theta_bias = 1.35f;
float x_bias = 0, y_bias = 0;

// Local meters conversion
...
// Build F (Jacobian of f wrt state) and fx (predicted state)
// State x = [ x, y, vx, vy ]
// Inputs   : ax_w, ay_w, dt
...
void setup() {
  ...
  // Initialize Serial1 for the BNO08x
  Serial1.begin(3000000, SERIAL_8N1, BNO_RX, BNO_TX); 
...
void loop() {
  ...
  float print_IMU_x = 0, print_IMU_y = 0, print_GPS_x = 0, print_GPS_y = 0;
...
//      Serial.println(az);
      ...
      print_IMU_x = ax_world;
      print_IMU_y = ay_world;
...
//    Serial.print("  gps_y_m: "); Serial.println(gps_y_m, 7);
    ...
    if (isnan(gps_x_m) || isnan(gps_y_m) || (!isfinite(gps_x_m) || !isfinite(gps_y_m))) {
      //Serial.println("GPS coords NaN after ll_to_local_m"); ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      lastGPS = millis();
      return;
    }
...
//    }
    ...
    if (Serial.available() >= 6) { // 4 bytes float + 2 bytes short
      ...
      char message[36];
      sprintf(message, "%06.2f%06.2f%06.2f%06.2f%04.0f%04hd%03.0f%05.2f%05.2f%03.0f%03.0f", ekf.x[0], ekf.x[1], ekf.x[2], ekf.x[3], steering, power, heading_deg, print_IMU_x, print_IMU_y, print_GPS_x, print_GPS_y);
